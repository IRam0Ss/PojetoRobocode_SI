...
import movement.waveSurfing.BulletWave;
import movement.waveSurfing.Enemy;
import movement.waveSurfing.WaveSurfer;
...
public class KillBot extends AdvancedRobot {
    ...
    public void run() {
        ...
        DefView.setColors(this);

        setAdjustGunForRobotTurn(true); // O canhão não gira com o corpo
        setAdjustRadarForGunTurn(true); // O radar não gira com o canhão (ou corpo)
        setAdjustRadarForRobotTurn(true); // O radar não gira com o corpo

        movementControl = new MovementControl(this);
        waveSurfer = new WaveSurfer(this, activeWaves, movementControl);

        while (true) {
            out.println("====== Turno: " + getTime() + " | Energia: " + getEnergy() + " | Pos: ("
                    + String.format("%.2f", getX()) + ", " + String.format("%.2f", getY()) + ") ======");
    ...
    public void onScannedRobot(ScannedRobotEvent e) {
        ...
        // --- 1. Radar Lock Aprimorado (Onda de Varredura - Scanning Wave) ---
        // Calcula o ângulo absoluto do inimigo em relação ao seu robô e, a partir
        // disso,
        // o ângulo que o radar precisa virar para apontar para o inimigo.
        double absoluteBearing = getHeadingRadians() + e.getBearingRadians();
        double radarTurn = absoluteBearing - getRadarHeadingRadians();

        // Multiplica por um fator (2.0) para fazer o radar "varrer" a área onde o
        // inimigo pode estar.
        // Isso ajuda a garantir que o radar não perca o alvo, mesmo se ele se mover
        // rapidamente.
        setTurnRadarRightRadians(normalRelativeAngle(radarTurn) * 2.0); // Multiplicador de 2.0 para agressividade
        ...
        if (energyDrop > 0.2 && energyDrop <= 3.0 && targetEnemy.getLastEnemyEnergy() > 0) {
            ...
            double angleOffset = normalRelativeAngle(absoluteBearingRadians - getHeadingRadians());
    ...
    public void onHitByBullet(HitByBulletEvent e) {
        ...
        if (hittingWave != null) {
            ...
            double angleOffset = normalRelativeAngle(hitAngle - hittingWave.getDirectAngle());
